...
<div id="wechat_menu">
	...
	<div class="clearfix" ng-controller="menuDisplay" id="menuDisplay">
		<div class="we7-margin-bottom clearfix">
			...
			<ul class="btn-group we7-btn-group" style="margin-bottom: 0;">
				<a href="<?php  echo url('platform/menu/post');?>" class="btn">当前默认菜单</a>
				<a href="<?php  echo url('platform/menu/display');?>" class="btn active">历史菜单列表</a>
			</ul>
			<?php  } ?>
			...
			<tr>
				...
				<td>
					<div class="link-group">
						...
						<a href="<?php  echo url('platform/menu/delete', array('id' => $da['id']));?>" class="del" onclick="if(!confirm('<?php  if($da['type'] == MENU_CURRENTSELF) { ?>删除默认菜单会清空所有菜单记录，确定吗？<?php  } else { ?>确定删除菜单?<?php  } ?>')) return false;">删除</a>
						<?php  } ?>
					</div>
				</td>
			</tr>
	...
	<ul class="btn-group we7-btn-group we7-margin-bottom">
		<a href="<?php  echo url('platform/menu/post');?>" class="btn active">当前默认菜单</a>
		<a href="<?php  echo url('platform/menu/display');?>" class="btn">历史菜单列表</a>
	</ul>
								...
								<div class="we7-form we7-padding-top">
									...
									<div class="form-group" ng-show="context.activeItem.forceHide == 0">
										...
										<div class="form-controls col-sm-10">
											...
											<?php  if(in_array($_W['account']['type'], array(ACCOUNT_TYPE_XZAPP_NORMAL, ACCOUNT_TYPE_XZAPP_AUTH)) || $_W['account']['level'] != ACCOUNT_SUBSCRIPTION) { ?>
