...
<div class="we7-page-title">APP管理</div>
<ul class="we7-page-tab">
	<li class="active"><a href="<?php  echo url('account/manage', array('account_type' => ACCOUNT_TYPE_PHONEAPP_NORMAL))?>">APP列表</a></li>
	

	
		<?php  if($_W['role'] == ACCOUNT_MANAGE_NAME_OWNER || $_W['role'] == ACCOUNT_MANAGE_NAME_FOUNDER) { ?>
		<li><a href="<?php  echo url ('account/recycle', array('account_type' => ACCOUNT_TYPE_PHONEAPP_NORMAL))?>">APP回收站</a></li>
		<?php  } ?>
	

</ul>
<div class="clearfix we7-margin-bottom">
	<?php  if(!$_W['isfounder'] && !empty($account_info['phoneapp_limit']) || user_is_vice_founder()) { ?>
	...
	<form action="" class="form-inline  pull-left" method="get">
		...
		<div class="pull-right">
			<a href="<?php  echo url('phoneapp/manage/create_display');?>" class="btn btn-primary we7-padding-horizontal">添加APP</a>
		</div>
		<?php  } ?>
	...
	<tr class="color-gray" ng-repeat="list in lists">
		...
		<td class="vertical-middle table-manage-td">
			<div class="link-group">
				<a ng-href="{{links.switch}}uniacid={{list.uniacid}}&type={{list.type}}">进入APP</a>
				<?php  if($role_type) { ?>
			...
			<div class="manage-option text-right">
				...
				<a href="{{links.postVersion}}&do=display&uniacid={{list.uniacid}}&acid={{list.acid}}&account_type={{list.type}}">版本管理</a>
				...
				<a ng-href="{{links.del}}&acid={{list.acid}}&uniacid={{list.uniacid}}&account_type={{list.type}}" ng-show="list.role == 'owner' || list.role == 'founder'" onclick="if(!confirm('确认放入回收站吗？')) return false;" class="del">停用</a>
				<?php  } ?>
...
<script>
	...
	links: {
		switch: switch_url,
			...
			postVersion: "<?php  echo url('phoneapp/manage')?>",
